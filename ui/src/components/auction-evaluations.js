...
function toAuction(stored, credits) {
    if (stored == null)
        return null;
    return Math.floor((stored * credits) / BASE_CREDITS);
}
function toBase(displayed, credits) {
...
let AuctionEvaluations = class AuctionEvaluations extends LitElement {
    constructor() {
        super(...arguments);
        this.players = [];
        this.status = null;
    ...
    updated(changed) {
        if (changed.has("auction")) {
            const prev = changed.get("auction");
    ...
    async load() {
        ...
        try {
            ...
            if (!statusRes.ok)
                throw new Error(`/evaluations/status HTTP ${statusRes.status}`);
    ...
    async refreshStatus() {
        try {
            const res = await fetch(`${BACKEND_URL}/auctions/${this.auction.id}/evaluations/status`);
            if (!res.ok)
                return;
    ...
    get startBlockers() {
        ...
        if (!s)
            return ["loading…"];
        const out = [];
        if (s.credits.status !== "ok")
            out.push(`credits ${s.credits.status}`);
        if (s.players.status !== "ok")
            out.push(`players ${s.players.status}`);
        if (s.goalkeepers.status !== "ok")
            out.push(`goalkeepers ${s.goalkeepers.status}`);
        return out;
    }
    get filteredSorted() {
        ...
        const filtered = this.players.filter((p) => (!team || p.team === team) &&
    ...
    async save(playerId, raw) {
        const trimmed = raw.trim();
        const displayed = trimmed === "" ? null : Number.parseInt(trimmed, 10);
        if (displayed !== null && (Number.isNaN(displayed) || displayed < 0))
            return;
        const stored = displayed === null ? null : toBase(displayed, this.auction.credits_per_team);
        try {
            const res = await fetch(`${BACKEND_URL}/auctions/${this.auction.id}/evaluations/${playerId}`, {
                method: "PATCH",
            ...
            this.players = this.players.map((p) => p.id === playerId ? { ...p, evaluation: updated.evaluation } : p);
    ...
    exportCsv() {
        window.open(`${BACKEND_URL}/auctions/${this.auction.id}/evaluations/export`, "_blank", "noopener");
    }
    triggerImport() {
        const input = this.renderRoot.querySelector('input[type="file"][data-csv]');
        input?.click();
    }
    async onImportFile(event) {
        ...
        if (!file)
            return;
        if (!confirm(`Import evaluations from "${file.name}" into "${this.auction.name}"?\n` +
        ...
        fd.append("file", file);
        try {
            const res = await fetch(`${BACKEND_URL}/auctions/${this.auction.id}/evaluations/import`, { method: "POST", body: fd });
    ...
    async startAuction() {
        if (!this.allGreen)
            return;
        if (!confirm(`Start auction "${this.auction.name}"? Evaluations will be locked once the auction is IN_PROGRESS.`)) {
            return;
        }
        this.starting = true;
        this.startError = "";
        try {
            ...
            this.dispatchEvent(new CustomEvent("auction-started", {
    ...
    renderIndicator() {
        ...
        const barWidth = (p) => `${Math.min(p, 100)}%`;
        const metric = (label, value, kind, percentage) => {
      ...
      <div class="border border-slate-200 rounded-md p-3 bg-slate-50 max-w-xl flex flex-col gap-2">
        ...
        ${metric("Credits spent", `${s.credits.used} / ${s.credits.total}`, s.credits.status, s.credits.percentage)}
        ${metric("Players evaluated", `${s.players.evaluated} / ${s.players.min}–${s.players.max}`, s.players.status, s.players.percentage)}
        ${metric("Goalkeepers evaluated", `${s.goalkeepers.evaluated} / ${s.goalkeepers.target}`, s.goalkeepers.status, s.goalkeepers.percentage)}
      </div>
    `;
    }
    headerCell(label, key) {
      ...
      <tr class="border-b border-slate-100">
        ...
        <td class="px-3 py-1.5 whitespace-nowrap">
          <input
            ...
            @change=${(e) => this.save(p.id, e.target.value)}
    ...
    render() {
        if (this.loading)
            return html `<p class="text-slate-600">Loading…</p>`;
        if (this.loadError)
        ...
        <div class="flex items-center gap-3 flex-wrap text-sm">
          <label class="flex items-center gap-2">
            ...
            <select
              ...
              @change=${(e) => (this.teamFilter = e.target.value)}
          ...
          <label class="flex items-center gap-2">
            ...
            <select
              ...
              @change=${(e) => (this.roleFilter = e.target.value)}
              class="rounded border border-slate-300 px-2 py-1"
            >
              <option value="">All</option>
          ...
          <label class="flex items-center gap-2">
            ...
            <input
              ...
              @input=${(e) => (this.nameFilter = e.target.value)}
              class="rounded border border-slate-300 px-2 py-1"
            />
          </label>
...
], AuctionEvaluations.prototype, "auction", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "players", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "status", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "loading", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "loadError", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "teamFilter", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "roleFilter", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "nameFilter", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "sortKey", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "sortDir", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "saveErrors", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "csvBusy", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "csvMessage", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "csvMessageKind", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "starting", void 0);
__decorate([
    state()
], AuctionEvaluations.prototype, "startError", void 0);
AuctionEvaluations = __decorate([
    customElement("auction-evaluations")
], AuctionEvaluations);
export { AuctionEvaluations };
